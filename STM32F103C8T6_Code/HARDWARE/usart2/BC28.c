...
{ 
	 if(Time4_Count%200==0)
    u1_printf(buf_uart2buf);  //清空前打印信息
	  Check_TCP();
    Delay(30);
    buf_uart2index=0;
...
int BC28_Init(void)
{
    int errcount = 0;
    err = 0;    //判断模块卡是否就绪最重要
    u1_printf("start init bc28\r\n");
    Uart2_SendStr("ATE1\r\n");
    Delay(30);
    u1_printf(buf_uart2buf);      //打印收到的串口信息
    u1_printf("get back bc28\r\n");
...
void BC28_PDPACT(void)//激活场景，为连接服务器做准备
{
    ...
    u1_printf("BC28_PDPACT sever OK  \r\n");  //清空前打印信息
}
